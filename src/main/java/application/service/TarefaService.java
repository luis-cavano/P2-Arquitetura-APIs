...
public class TarefaService {
    ...
    public TarefaDTO findById(long id) {
        ...
        if (resultado.isEmpty()) {
            throw new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "Tarefa não localizada");
        }
    ...
    public void insertColaborador(Long tarefaId, Long colaboradorId) {
        ...
        if (tarefa == null) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Tarefa não localizada");
        }

        Colaborador colaborador = colaboradorRepository.findById(colaboradorId).orElse(null);
        if (colaborador == null) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Colaborador não localizado");
        }
